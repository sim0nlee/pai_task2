...
class SGDHinton(Optimizer):
    ...
    def __init__(self, params, lr, momentum=0.0, l2_limit=15.):
        if lr < 0.0:
...
class BackpropTrainer(Framework):
    ...
    def predict_probabilities(self, x: torch.Tensor, num_mc_samples: int = 10) -> torch.Tensor:
        ...
        """
        probability_samples = torch.stack([F.softmax(self.network(x), dim=1) for _ in range(num_mc_samples)], dim=0)
